...
function match(element, selector) {
  ...
  if (tagSelector !== null) {
    ...
    if (idSelectors !== null) {
      let attr = element.attributes.filter(attr => attr.name === 'id')[0];
      if (attr && attr.value !== idSelectors.join(' ')) return false;
    }

    if (classSelectors !== null) {
      let attr = element.attributes.filter(attr => attr.name === 'class')[0];
      if (attr && attr.value !== classSelectors.join(' ')) return false;
    }
  } else if (idSelectors !== null) {
    let attr = element.attributes.filter(attr => attr.name === 'id')[0];
    if (attr && attr.value !== idSelectors.join(' ')) return false;
  } else if (classSelectors !== null) {
    let attr = element.attributes.filter(attr => attr.name === 'class')[0];
    if (attr && attr.value !== classSelectors.join(' ')) return false;
  }
